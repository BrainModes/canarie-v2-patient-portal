...
def rds_connection_pool_factory():
    ...
    try:
        ...
        print("[INFO]", 'Initiated RDS Pool')
        return my_pool
    except Exception as e:
        print("[FATAL]", "Error when initiating RDS pool: ", str(e))
        raise
...
def postgre_simple_querier(conn, query, iffetch=True, iffetchone=False ):
    ...
    try:
        ...
        conn.rollback()
        cursor.close()
        print('[POSGRE_SIMPLE_QUERY]: ', 'Failed', query)
        print('[POSGRE_SIMPLE_QUERY]: Robacked------', e)
        raise
